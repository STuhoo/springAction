...
public class Car implements BeanFactoryAware, BeanNameAware, InitializingBean, DisposableBean {
	...
	public Car() {
		System.out.println("调用Car()构造函数。"+name+brand);
	}
	...
	public void setBeanFactory(BeanFactory beanFactory) throws BeansException {
		System.out.println("CAR接口实现：调用BeanFactoryAware.setBeanFactory()。");
		this.beanFactory = beanFactory;
	}

	// BeanNameAware接口方法
	public void setBeanName(String beanName) {
		System.out.println("CAR接口实现：调用BeanNameAware.setBeanName()。"+beanName);
		this.beanName = beanName;
	}

	// InitializingBean接口方法
	public void afterPropertiesSet() throws Exception {
		System.out.println("CAR接口实现：调用InitializingBean.afterPropertiesSet()。");
	}
